...
void main() {
  ...
  testWidgets('glass reading overlapping strips shares one capture a frame',
    ...
    await tester.pumpWidget(_frame(Stack(
      ...
      children: <Widget>[
        Positioned.fill(
          child: BackdropLayer(backdrop: backdrop, child: _stripes(controller)),
        ),
  ...
  testWidgets('a repaint that lands nowhere under the glass is not re-captured',
      (WidgetTester tester) async {
      ...
      children: <Widget>[
        Positioned.fill(
          child: BackdropLayer(
            ...
            child: Column(
              children: <Widget>[
                SizedBox(height: 60, width: _w.toDouble(), child: _pulse(t)),
    ...
    expect(source.debugIgnoredChanges, greaterThanOrEqualTo(steps),
  ...
  testWidgets('the same repaint under the glass is followed',
    ...
    expect(source.debugCaptureCount, steps,
        reason: 'each change under the glass is one re-capture, no more');
  });

  testWidgets('a scroll that lands nowhere under the glass is not re-captured',
      (WidgetTester tester) async {
    ...
    expect(source.debugCaptureCount, 0,
        reason: 'a carousel scrolling 140 pixels above the glass cost it a '
            're-capture');
  });
  ...
  testWidgets(
    ...
    await tester.pump();
    await tester.pump();
    expect(_source(tester).debugLastCapturePixelRatio, 1.0,
    ...
    await tester.pump();
    expect(source.debugCaptureCount, frames + 1,
