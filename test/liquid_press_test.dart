...
LiquidPressDeformation _press({
  ...
  double progress = 0.0,
    ...
    LiquidPressDeformation.resolve(size, offset: offset, pressProgress: progress);
...
void main() {
  ...
  test('it leans towards the finger, and tanh bounds the lean', () {
    final LiquidPressDeformation near = _press(offset: const Offset(20, 0), progress: 1.0);
    expect(near.translationX, greaterThan(0.5));
    expect(near.translationX, lessThan(20.0),
    ...
    final LiquidPressDeformation far = _press(offset: const Offset(100000, 0), progress: 1.0);
  ...
  test('the lean is signed, and each axis is its own', () {
    final LiquidPressDeformation up = _press(offset: const Offset(-20, -30), progress: 1.0);
  ...
  test('a horizontal drag stretches across, not down', () {
    final LiquidPressDeformation held = _press(progress: 1.0);
    final LiquidPressDeformation pulled =
        _press(offset: const Offset(60, 0), progress: 1.0);
  ...
  test('a vertical drag stretches down, not across', () {
    final LiquidPressDeformation held = _press(progress: 1.0);
    final LiquidPressDeformation pulled =
        _press(offset: const Offset(0, 60), progress: 1.0);
    expect(pulled.scaleY, greaterThan(held.scaleY));
    expect(pulled.scaleX, closeTo(held.scaleX, 1e-9));
  });

  test('the matrix is the four numbers, scaled about the centre by Transform', () {
    final LiquidPressDeformation press =
        _press(offset: const Offset(24, 8), progress: 1.0);
