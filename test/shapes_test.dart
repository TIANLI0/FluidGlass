...
void main() {
  group('RoundedRectangle', () {
    test('clamps its radius to half the shortest side', () {
      ...
      final RectangleCorners corners = shape.corners(
        const Size(40, 200),
        TextDirection.ltr,
      );
    ...
    test(
      'uses an RRect for circular corners and a path for continuous ones',
      () {
    ...
    test('continuous outline stays inside its bounds and spans them', () {
      ...
      final GlassOutline outline = const RoundedRectangle(
        32,
      ).createOutline(size, TextDirection.ltr);
    ...
    test(
      'a continuous corner blends into the edge later than a circular one',
      () {
        ...
        const Size size = Size(200, 120);
        final Path continuous = const RoundedRectangle(
          40,
        ).createOutline(size, TextDirection.ltr).toPath();
        final Path circular = const RoundedRectangle(
          40,
          style: RoundedCornerStyle.circular,
        ).createOutline(size, TextDirection.ltr).toPath();
        ...
        final double continuousDiagonal = firstInside(
          continuous,
          (double t) => Offset(t, t),
        );
        final double circularDiagonal = firstInside(
          circular,
          (double t) => Offset(t, t),
        );
        expect(continuousDiagonal, closeTo(circularDiagonal, 0.2));

        final double continuousEdge = firstInside(
          continuous,
          (double t) => Offset(t, 0.6),
        );
        final double circularEdge = firstInside(
          circular,
          (double t) => Offset(t, 0.6),
        );
  ...
  group('Capsule', () {
    test('rounds by half the shortest side', () {
      final RectangleCorners corners = const Capsule().corners(
        const Size(120, 40),
        TextDirection.ltr,
      );
      expect(corners.topLeft, 20);
  ...
  group('UnevenRoundedRectangle', () {
    test('resolves start/end against the text direction', () {
      final UnevenRoundedRectangle shape = UnevenRoundedRectangle.only(
        topStart: 10,
        bottomEnd: 20,
      );
  ...
  group('Rectangle', () {
    test('has no corners', () {
      final RectangleCorners corners = const Rectangle().corners(
        const Size(10, 10),
        TextDirection.ltr,
      );
